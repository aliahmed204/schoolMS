                ...
                <div class="col-lg-4 ">
                    <div class="card mb-4 mt-4 ">
                        <div class="card-body text-center">
                            ...
                            <h5 style="font-family: Cairo,emoji" class="my-3">{{$teacher->name}}</h5>
                            <p class="text-muted mb-1">{{$teacher->email}}</p>
                            <p class="text-muted mb-2">{{trans('Teacher_trans.teacher')}}</p>
                            <p class="text-muted mb-2">{{trans('Teacher_trans.Joining_Date')}}: {{$teacher->joining_date}}</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-8">
                    <div class="card mb-4 mt-4 ">
                        <div class="card-body">
                            <form action="{{route('teacher.profiles.update',$teacher->id)}}" method="post">
                                @csrf
                                @method('patch')
                                <div class="row">
                                    <div class="col-sm-3">
