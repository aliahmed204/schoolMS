...
@section('content')
    ...
    <form method="POST" action="{{ route('teacher.students.attendances')}}">
        @csrf
                    ...
                    <td>
                            ...
                            <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
                                <input name="attendences[{{ $student->id }}]"
                                       @foreach($student->attendance()->where('for_day','=',date('Y-m-d'))->get() as $attendance )
                                           @checked($attendance->status == 1 )
                                       @endforeach
